...
*/
	...
	function qa_lang_sub($identifier, $textparam, $symbol='^', $reverse=false)
/*
	Return the translated string for $identifier, with $symbol substituted for $textparam
*/
	{
        if($reverse){
            $str = str_replace($symbol, $textparam, qa_lang($identifier));
            $str = explode(" ", $str);
            $str[count($str) - 1] = '<span>'.$str[count($str) - 1].'</span>';
