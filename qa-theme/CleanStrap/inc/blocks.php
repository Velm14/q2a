...
class qa_html_theme extends qa_html_theme_base
{
    ...
    function head_script()
    {
        ...
        //    document.write("<script src='/qa-theme/CleanStrap/js/iscroll.js'></script>");

        $this->output('<script type="text/javascript" src="' . Q_THEME_URL . '/js/jquery.sparkline.min.js"></script>');
        $this->output('<script type="text/javascript" src="' . Q_THEME_URL . '/js/jquery.sparkline.min.js"></script>');

        $this->output('<script type="text/javascript" src="' . Q_THEME_URL . '/js/iscroll.js"></script>');
...
?>
    ...
    function q_item_main($q_item)
    {
        ...
        if (!(bool) qa_opt('cs_enable_clean_qlist')) {
            ...
            qa_html_theme_base::answer_count($q_item['raw']['acount']);
//            $q_item['raw']['acount']
    ...
    function footer()
    {
        ...
        $this->output('<footer id="site-footer" class="clearfix">');
			 ...
			 $this->output('<div class="vverh">');
			echo '	
<a href="javascript:void(0)"
   onclick="scroll(0,0)">Вверх</a>';
	$this->output('</div>');
		...
		$this->nav('footer');
		  ...
		  $this->output('<div class="center-cop">');
			echo '<span>© «Спроси & Ответь», 2014 </span>';
 $this->output('</div>');
...
//            $this->output(base64_decode(qa_opt('cs_ads_after_question_content')));
    ...
    function q_view_main($q_view)
    {
        ...
        if (strpos($this->content['title'], $q_view['raw']['title'])) {
            ...
            $this->q_view_close($q_view);

            $this->output('<div style="background: #eee; padding-top: 15px;">');
            $this->output('<div style="background: white; padding: 10px">');
            $this->output(base64_decode(qa_opt('cs_ads_after_question_content')));
            $this->output('</div>');
            $this->output('</div>');
	...
	function q_view_close($q_view)
	{
		if (!empty($q_view['close'])) {
			$haslink=isset($q_view['close']['url']);
			
			$this->output(
				'<div class="qa-q-view-close icon-cross">',
				$q_view['close']['label'],
				($haslink ? ('<a href="'.$q_view['close']['url'].'"') : '<span').' class="qa-q-view-close-content">',
				$q_view['close']['content'],
				$haslink ? '</a>' : '</span>',
				'</div>'
			);
		}
    ...
    function ra_post_buttons($q_view, $show_feat_img=false)
    {
        ...
        $this->output('<div class="post-button btn-group clearfix">');
        ...
        $this->output('<div class="btn-group" style="float: right">');
        $this->output('<button type="button" class="btn btn-default dropdown-toggle" data-toggle="dropdown">
					Меню <span class="caret"></span>
					</button>');
        $this->output('<div class="dropdown-menu row span1"><ul>');
        foreach ($buttons as $k => $btn) {
            if ($k == 'edit')
                $btn['class'] = 'icon-edit';
            if ($k == 'flag')
            ...
            $this->output('<li><button ' . $btn['tags'] . ' class="btn ' . @$btn['class'] . '" title="' . $btn['popup'] . '" type="submit">' . $btn['label'] . '</button></li>');
        }
        $this->output('</ul></div>');
        $this->output('</div>');
//		if (($this->template == 'question') && (qa_get_logged_in_level() >= QA_USER_LEVEL_ADMIN) && (!empty($q_view)) && $show_feat_img){
...
//		}
    ...
    function c_list_item($c_item)
    {
        ...
        $this->output('<div class="my-border" style="display: none; margin-top: 10px;
  padding: 5px 0; border-top: 1px solid #F7F7F7; height: 1px"></div>');
    ...
    function a_item_main($a_item)
    {
		...
		$username = ($a_item['raw']['handle']) ? cs_name($a_item['raw']['handle']) : 'Аноним';
        $askerPoint = ($a_item['who']['points']) ? '<p class="asker-point">' . implode(' ', $a_item['who']['points']) . ' <span class="title">' . $a_item['who']['level'] . '</span></p>' : '';
        $this->output('
					<div class="user-info no-overflow">
						<h3 class="asker-name">' . $username . '</h3>
						'.$askerPoint.'
...
//            $this->cs_hover_button($post, 'select_tags', qa_lang('cleanstrap/select_answer'), 'icon-checkmark3 qa-a-select' );
            $this->cs_hover_button($post, 'select_tags', '<img src="/qa-theme/CleanStrap/images/zv.png">', 'icon-checkmark3 qa-a-select' );
//            var_dump($post);
        }
